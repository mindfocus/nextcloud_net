...
public class ContactsStore : IContactsStore {
	 ...
	 */
	public ContactsStore(IManager contactsManager,
								IConfig config,
	 ...
	 * @return IEntry[]
	 */
	public IList<IEntry> getContacts(IUser user, string filter) {
		var allContacts = this.contactsManager.search(filter ?: "", [
			"FN",
			"EMAIL"
		]);

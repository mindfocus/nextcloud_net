...
namespace OC.User
{
	...
	public Manager(IConfig config, ext.Event dispatcher, ILogger logger, CommonService _commonService)
	{
		this.logger = logger;
		this.config = config;
	 ...
	 */
    public IUser checkPassword(string loginName, string password)
    {
		var result = this.checkPasswordNoLogging(loginName, password);
        if (result == null) {
	        this.logger.warning("Login failed: \'" + loginName + "\' (Remote IP: \'" + _commonService.getRemoteAddress() + "\'",
			        new Dictionary<string, object>{{"app", "core"}});
        }
	 ...
	 * @throws \InvalidArgumentException
	 */
	public IUser createUserFromBackend(string uid, string password, UserInterface backend) {
	...
	public IUser[] getByEmail(string email) {
		var userIds = this.config.getUsersForUserValueCaseInsensitive('settings', 'email', email);
